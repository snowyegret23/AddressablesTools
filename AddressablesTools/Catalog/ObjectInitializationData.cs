...
namespace AddressablesTools.Catalog
{
    public class ObjectInitializationData
    {
        ...
        {
            reader.ValidateRange(offset, 12);
            ...
            if (objectTypeOffset != uint.MaxValue)
            {
                ObjectType = new SerializedType();
        ...
        internal uint Write(CatalogBinaryWriter writer)
        {
            ...
            BinaryPrimitives.WriteUInt32LittleEndian(bytes[4..], ObjectType?.Write(writer) ?? uint.MaxValue);
